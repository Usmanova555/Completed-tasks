...
while True:
    ...
    if f.startswith(tuple(["ls", "dir"])):
        ...
        if now != None: os.mkdir(now + '\\' + f.split()[1])
        ...
        else: print(Style.BRIGHT + Fore.RED + f"No such file with name {command.split()[1]}")
    elif f.startswith("ren"):
        if f.split()[1] in os.listdir(now):
            os.rename(f.split()[1], f.split()[2])
        else: print(Style.BRIGHT + Fore.RED + f"No such file with name {command.split()[1]}")
        ...
        else: print(Style.BRIGHT + Fore.RED + f"No such file or directory with name {command.split()[1]}. Please, don't use spaces")
        ...
        if f.split()[1].startswith('..'):
            ...
            for k in range(len(ff)):
                if ff[i] in os.listdir(now) and os.path.isdir(f"{temp[i]}"):
                    if now != None: now += '\\' + ff[k]
                    else: now = ff[i]
                else: print(Style.BRIGHT + Fore.RED + f"No such directory with name {temp[i]}. Please, don't use spaces")
        else:
